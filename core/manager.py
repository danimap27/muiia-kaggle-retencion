...
def do_check(cfg: dict, phase: Optional[dict] = None, view_only: bool = False):
    """
...
def main():
    ...
    while True:
        ...
        if c == "R":
            ...
            mode = do_check(cfg)
            if mode is not None:
                do_full_pipeline(cfg, slurm_sh, conda_env, overwrite=(mode == "overwrite_all"))

        elif c == "M":
            do_monitor(cfg)

        elif c == "C":
            do_check(cfg, view_only=True)
            ...
            mode = do_check(cfg, phase=p)
            if mode is not None:
                do_submit(p, slurm_sh=slurm_sh, conda_env=conda_env, overwrite=(mode == "overwrite_all"))
                input("\nEnter to return...")
